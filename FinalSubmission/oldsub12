...
main = do
        ...
        writeFile "tmp1.txt" (replaceourkey (cleanser file 0 ""))
        filetemp <- readFile "tmp1.txt"
        writeFile "tmp2.txt" (replaceWithKey (finalkey filetemp dict) filetemp)
        filetemp1 <- readFile "tmp2.txt"
        writeFile "tmp3.txt" (replaceWithKey (nayawordbyword (split filetemp1) [(' ',' ')] (split dict) 0) filetemp1)
        filetemp2 <- readFile "tmp3.txt"
        ...
        -- (replaceourkeycapital filetemp3)
        writeFile "answer2.txt" (replaceWithKey (nayawordbyword (split (replaceourkeycapital filetemp3)) [(' ',' ')] (split dict) 0) (replaceourkeycapital filetemp3))
        filetemp4 <- readFile "answer2.txt"
        writeFile "answer3.txt" (replaceWithKey (nayawordbyword (split filetemp4) [(' ',' ')] (split dict) 0) filetemp4)
        filetemp5 <- readFile "answer3.txt"
        writeFile "answer.txt" (replaceWithKey (nayawordbyword (split filetemp5) [(' ',' ')] (split dict) 0) filetemp5)
